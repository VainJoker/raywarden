...
pub async fn config(State(state): State<AppState>) -> Json<Value> {
    ...
    //     parse_experimental_client_feature_flags(&crate::CONFIG.
    // experimental_client_feature_flags()); feature_states.insert("
    // duo-redirect".to_string(), true); feature_states.insert("
    // email-verification".to_string(), true); feature_states.insert("
    // unauth-ui-refresh".to_string(), true); feature_states.insert("
    // enable-pm-flight-recorder".to_string(), true); feature_states.
    // insert("mobile-error-reporting".to_string(), true);

    let disable_user_registration = state.config.disable_user_registration;
    let domain = state.get_domain().to_string();
